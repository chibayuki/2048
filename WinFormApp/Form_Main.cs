...
namespace WinFormApp
{
    public partial class Form_Main : Form
    {
        ...
        private static readonly string URL_GitHub_Base = @"https://github.com/chibayuki/Com"; // 此项目在 GitHub 的 URL。
                ...
                try
                {
                    ...
                    while (SubStr.IndexOf("{") != -1 && SubStr.IndexOf("}") != -1)
                    {
                        try
                        {
                            ...
                            Step S = new Step();
                            S.Array = new Int32[CAPACITY, CAPACITY];

                            //

                            string StepString_Array = Com.Text.GetIntervalString(StepString, "[", "]", false, false);

                            while (StepString_Array.IndexOf("(") != -1 && StepString_Array.IndexOf(")") != -1)
                            {
                                try
                                {
        ...
        private void _SaveGameStep()
        {
            ...
            Str += "<Probability>" + Probability + "%</Probability>";
            Str += "<EnableUndo>" + EnableUndo + "</EnableUndo>";
            ...
            Str += "<Score>" + ThisRecord.Score + "</Score>";
            ...
            if (EnableUndo && SaveEveryStep)
            {
                    ...
                    for (int i = 0; i < StepList_Previous.Count; i++)
                    {
                        ...
                        for (int X = 0; X < Range.Width; X++)
                        {
                            for (int Y = 0; Y < Range.Height; Y++)
                            {
                                if (S.Array[X, Y] != 0)
                                {
                                    StepListString += "(" + X + "," + Y + "," + S.Array[X, Y] + ")";
                                }
                            }
                        }
                        StepListString += "]";

                        StepListString += "(" + S.Score + "," + (Int32)S.Direction + "," + S.Index.X + "," + S.Index.Y + "," + S.Value + ")";
                ...
                if (StepList_Next.Count > 0)
                {
                    ...
                    for (int i = 0; i < StepList_Next.Count; i++)
                    {
                        ...
                        StepListString += "(" + (Int32)S.Direction + "," + S.Index.X + "," + S.Index.Y + "," + S.Value + ")";
                    }
                    StepListString += "</Next>";
                }
            }
        ...
        private Point GetRandomZeroIndexOfArray(Int32[,] Array, Size Cap)
        {
            //
        ...
        private enum InterruptActions { NULL = -1, StartNew, Continue, Previous, Next, Restart, Exit, CloseApp, COUNT } // 中断动作枚举。
        ...
        private Graphics ProbabilityTrbBmpGrap; // 概率衰减调节器位图绘图。
        ...
        private void UpdateProbabilityTrbBmp()
        {
            ...
            ProbabilityTrbBmpGrap = Graphics.FromImage(ProbabilityTrbBmp);
